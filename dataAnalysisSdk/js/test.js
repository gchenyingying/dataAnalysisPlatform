...
function getOSInfo(){
	...
	//console.log(ua.search(/iphone|ipad|ipod/));
...
var cookieUntil={
	...
	remove:function(cname,path){
		this.set(cname,"",new Date(0),path);
	}
}
//city geolocation
function initialize() {
	...
	geolocation.getCurrentPosition(function(r){
		if(this.getStatus() == BMAP_STATUS_SUCCESS){
			...
			function myFun(result){
				cookieUntil.set("cityName",result.name,0,"/");
			}
...
function loadBaiduGeoApi(){
	...
	function loadScript() {  
  		...
  		script.src = "http://api.map.baidu.com/api?v=2.0&ak=3LQnukfDQBsEoSnfC1bx6UM3DmKjnBsR&callback=initialize";
...
$.dataAnalysis.pageDataHandler=function(){
	...
	console.log(pageInfo.timeStamp);
	//检测是否获取城市位置
...
$.widget("dataAnalysis.eventHandler",{
	...
	_clickHandler:function(event){
		if(this===event.target){
			//console.log(typeof $(this).data("dataAnalysis-eventHandler")._eventInfo);
	...
	_throttledExposureHandler:function(eventType){
		...
		//console.log("1:"+analysisElement.id);
		//元素初始状态在视口内部
		if(inViewport){
			var eventInfo=$(analysisElement).data("dataAnalysis-eventHandler")._eventInfo(eventType,false);
			//send(eventInfo);
			sendExposureStatus=true;//状态设置为已发送
			console.log("inView is in");				//alert("inView is in");
			console.log(eventInfo);
			//document.getElementById("test7").innerHTML=document.getElementById("test7").innerHTML+"inView is in";			
		}
		function exposureAnalysis(){
			console.log("2"+analysisElement.id);
			inViewport=isViewport(analysisElement);
			...
			if(sendExposureStatus&&!inViewport){
				...
				console.log("inView is out");
				//alert("inView is out");
				//document.getElementById("test7").innerHTML=document.getElementById("test7").innerHTML+"inView is out";
			}
		}
		var timerId,delayInterval=300;//节流
	...
	_eventInfo:function(eventType,containType){
		...
		switch(eventType)
		{
			...
			case "touchmove":
				...
				break;
			default:
				console.log("[eventInfo] event type error");
		}
