	...
	<el-table :data="list" stripe>
    	...
    	<el-table-column label="应用名称">
    		<template slot-scope="scope">
				<router-link :to="{ path: `/dataManageApp/${list[scope.$index].appId}` }">{{ list[scope.$index].appId }}</router-link>
    		</template>
    	</el-table-column>
    	<el-table-column prop="lastModifyTime" label="最近修改时间" width="200">
    	</el-table-column>
  ...
  methods:{
  	confirmPop:function(index){
		...
		this.list.forEach(function(item,index,array){
  		...
  		const options={  			
				method:"post",
				url:"/testDelDataApp",//for testing
				data:{
					dataAppId:this.list[index].dataAppId//抽取APP名称
				},
				success:(data)=>{
